<template>
    ...
    <div>
        <el-form ref="form" :model="form" label-width="100px" >
            ...
            <el-form-item label="垃圾名称：" prop="rubbishName">
                <el-input v-model="form.rubbishName" ></el-input>
            </el-form-item>
            <el-form-item label="垃圾类别：" prop="rubbishType">
                <el-select v-model="form.rubbishTypeNub" placeholder="请选择">
                    <el-option
            ...
            <el-form-item label="垃圾简介：" prop="rubbishIntro">
                <el-input v-model="form.rubbishIntro" type="textarea"  maxlength="50" show-word-limit></el-input>
            </el-form-item>
    ...
    <el-dialog
      width="30%"
      title="修改"
      :visible.sync="innerVisibleChange"
      append-to-body>
      此操作将修改垃圾, 是否继续?
        <span slot="footer" class="dialog-footer">
            <el-button type="warning" @click="ChangeRubbishInner()">确 定</el-button>
            <el-button @click="innerVisibleChange = false">取 消</el-button>
        </span>
    </el-dialog>
    ...
    <el-dialog
      width="30%"
      title="修改"
      :visible.sync="innerVisibleRecover"
      append-to-body>
      此操作将恢复垃圾, 是否继续?（你可以在“已发布”中找到）
        <span slot="footer" class="dialog-footer">
            <el-button type="warning" @click="RecoverRubbishInner(form.id)">确 定</el-button>
            <el-button @click="innerVisibleRecover = false">取 消</el-button>
        </span>
    </el-dialog>
    ...
    <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="innerVisiblePreview = true">预 览</el-button>
        <el-button type="success" @click="innerVisibleRecover = true">恢 复</el-button>
        <el-button type="warning" @click="innerVisibleChange = true">修 改</el-button>
...
export default {
    data(){
        ...
        return{
            ...
            dialogVisible: false,
            // 是否展示内嵌修改窗口
            innerVisibleChange: false,
            // 是否显示内嵌预览框
            innerVisiblePreview: false,
            // 是否显示内嵌恢复窗口
            innerVisibleRecover: false,
            // 编辑表单
            form: {
                id: '',
                authorId:'',
    ...
    methods: {
        ...
        handleClick(row) {
            ...
            this.form.rubbishTypeNub = row.rubbishTypeNub;
        ...
        // 内嵌窗口的恢复功能
        RecoverRubbishInner(id){
            this.dialogVisible= false;
            this.innerVisibleRecover = false;
            rubbishApi.changeStateById(id,1).then(response=>{
                this.$message({
                    message: response.message,
                    type: (response.flag ? 'success':'error')
        ...
        ChangeRubbishInner(){
            this.dialogVisible= false;
            this.innerVisibleChange = false;
            rubbishApi.changeRubbish(this.form).then(response=>{
                this.$message({
                message: response.message,
                type: (response.flag ? 'success':'error')
            });
